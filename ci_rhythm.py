...
from ci_search import ci_type_extraction, search_ci
from cipai_word_counts import qin_num, long_num
from common import *
...
class CiRhythm:
    def __init__(self, yun_shu: int, ci_pai_name: str, ci_content: str, ci_comma: str, give_type: str, ci_pu: int):
    ...
    def _ping_ze_right(self, cipai: str) -> list[str | bool]:
        ...
        for hanzi_num in range(len(self.ci_content)):
            ping_ze = hanzi_to_pingze(self.ci_content[hanzi_num], yun_shu)
            if ping_ze == '0':
                result.append('duo')
            elif ping_ze == '3':
                result.append('pi')
    ...
    @staticmethod
    def _show_ci(ge_lyu_final: list, text_final: list, yun_final: list, your_lyu_final: list) -> str:
        """
        将所有得到的结果组合称为最终的结果
        Args:
            ge_lyu_final: 词的格律
    ...
    def _build_single_ci_report(self, ci_num: str) -> str | int:
        ...
        if not self.ci_pai_name:
            best = search_ci(ci_num, self.ci_pu, reverse_search=True) + '\n' + best
        if warn:  # 只在这里加一次提示
            best = "给定格式与实际相差过大或没有此格式，将另行匹配。\n" + best
        return best
    ...
    def _one_format_report(self, ci_num: str, type_list: list, fmt_id: int) -> str:
        ...
        yun_nums = [hanzi_to_yun(self.ci_content[i], self.yun_shu, ci_lin=True)
                    for i in yun_pos]
        ...
        # 水龙吟格二十四特殊处理
        if fmt_id == 23 and ci_num == '658':
            report += (f'\n仄句\n{self.ci_content[-1]}\n'
                       f'{self.show_mark[int(hanzi_to_pingze(self.ci_content[-1], self.yun_shu))]}\n')
        return report
    ...
    def _fmt_yun_info(self, d: dict) -> str:
        ...
        group = f"第{num_to_cn(d['group'])}组韵"
        yayun = '' if d['is_yayun'] else '不'
        info = f'{hanzi_yun} {group} {yayun}押韵'
        return '不知韵部' if ('零' in info or len(info) == 9) else info
