...
def infer_sen_len(poem: str, has_comma: bool) -> int:
...
def infer_poem_type(total_lines: int) -> str:
    """总行数 -> 诗体中文名"""
    if total_lines == 4:
        return '绝句'
    if total_lines == 8:
...
def rhythm_to_pingze(rhythm: int, yun_shu: int) -> int:
    """韵部 -> 平仄标记"""
    if yun_shu == 1:
...
class ShiRhythm:
    def __init__(self, yun_shu, poem, poem_comma):
    ...
    def _lyu_ju(self, sentence: str, rule: int, poem_pingze: int,
        ...
        sentence_pattern = ''.join(hanzi_to_pingze(char, self.yun_shu) for char in sentence)
        ...
        """
            ...
            Returns:
                返回两个值：
                    ...
                    修正后的 second
            """
        last1 = hanzi_to_pingze(first_sen[-1], self.yun_shu)
        last3 = hanzi_to_pingze(first_sen[-3], self.yun_shu)
        if last1 not in ['0', '3']:
    ...
    def _sentence_show(self, show_sentence: str, sen_ge_lyu: list[bool]) -> str:
        ...
        for char in show_sentence:
            ping_ze = hanzi_to_pingze(char, self.yun_shu)
    ...
    def _special_two_pingze(self, hanzi1: str, hanzi2: str, poem_pingze: int) -> int:
        """
            ...
            Returns:
                第二个判断标准（两者平仄是否相同）
            """
        ping_ze1 = hanzi_to_pingze(hanzi1, self.yun_shu)
        if ping_ze1 == '3':
            ping_ze1 = '0'
        ping_ze2 = hanzi_to_pingze(hanzi2, self.yun_shu)
        if ping_ze2 == '3':
            ping_ze2 = '0'
    ...
    def _is_all_duo_yin(self, yun_jiao_content: str) -> bool:
        ...
        for i in yun_jiao_content:
            ping_ze = hanzi_to_pingze(i, self.yun_shu)
            if ping_ze != '0':
                return False
        return True
    ...
    def _build_report(self, maybe_len, main_rhythm, f_rhythm,
        ...
        """为单平仄方向生成完整报告"""
        sen_len = maybe_len or infer_sen_len(self.poem, self.poem_comma != self.poem)
        total_lines = len(self.poem) // sen_len
        poem_type = infer_poem_type(total_lines)

        report = f'{num_to_cn(sen_len)}言{poem_type}\n'

        s_rhythm = self._special_two_pingze(f_hanzi, s_hanzi, pingze)
        first_checker = ShiFirst(self.poem, self.yun_shu, s_rhythm, pingze, sen_len)
        ...
        for idx, rule in enumerate(rule_list):
            ...
            hint_buf += hint + '\u3000'
            sen_buf += sentence + '\u3000'
            ao_buf += (f'\n本联{"上" if idx % 2 == 0 else "下"}句' + ao) if ao else ''
    ...
    def main_shi(self) -> str | int:
        ...
        for maybe_len in candidates:
            ...
            rhythms = [hanzi_to_yun(y, self.yun_shu) for y in yun_jiaos]
            ...
            # 2.2 平仄标记
            pingze = rhythm_to_pingze(main_rhythm, self.yun_shu)
